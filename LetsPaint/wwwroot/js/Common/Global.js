...
api.get = function (url, options) {
    return new Promise(function (resolve, reject) {
        ...
        option.url = url.indexOf('http') === -1 && url.indexOf('www') === -1 ? `${url.baseUrl}` + url : url; `${url.baseUrl}` + url;
...
api.successMsgHandler = function (data) {
    ...
    else if (data.message === app.api.statusMsg.Blocked) {
        app.toast.success(app.userMsg.blocked);
    }
    else if (data.message === app.api.statusMsg.Unblocked) {
        app.toast.success(app.userMsg.unBlocked);
    }
    else {
